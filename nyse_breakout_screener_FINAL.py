...
class MobileBreakoutScreener:
    ...
    def calculate_indicators(self, data):
        ...
        # Volume Ratio with robust NaN handling
        # Fill NaN values in VolMA20 using bfill then forward fill
        df['VolMA20'] = df['VolMA20'].bfill().fillna(df['Volume'].mean())
        
        # Replace zeros with mean to avoid division by zero
        df['VolMA20'] = df['VolMA20'].replace(0, df['Volume'].mean())
        
        # Calculate volume ratio safely
        df['VolRatio'] = (df['Volume'] / df['VolMA20']).fillna(1.0)
        df['VolRatio'] = df['VolRatio'].replace([np.inf, -np.inf], 1.0)
